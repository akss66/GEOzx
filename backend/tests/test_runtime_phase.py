...
def test_normalizes_public_turn_phases_from_runtime_events() -> None:
    ...
    assert (
        normalize_runtime_phase("brain.runtime.tool_started", {"tool_code": "account.data_context"})
        == "reading_data"
    )
...
def test_explicit_public_turn_phase_wins_and_invalid_values_fail_closed() -> None:
    assert (
        normalize_runtime_phase("brain.runtime.tool_started", {"turn_phase": "quality_review"})
        == "quality_review"
    )
    assert (
        normalize_runtime_phase("brain.runtime.tool_started", {"turn_phase": "internal_magic"})
        == "reading_data"
    )
