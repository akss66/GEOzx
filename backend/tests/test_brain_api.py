...
@pytest.mark.asyncio
async def test_brain_task_followup_keeps_single_runtime_task_and_context(
    client,
    admin,
    session,
    monkeypatch,
):
    async def classify_followup(*_args, **_kwargs):
        return IntentDecision(
            intent="conversation",
            confidence=1,
            reason="deterministic compatibility-path conversation",
            suggested_expert_codes=[],
            requires_account_context=False,
    ...
    first = await client.post(
        "/brain/messages",
        headers=headers,
        json={"message": "Hello", "client_message_id": "runtime-first"},
    )
    ...
    second = await client.post(
        ...
        headers=headers,
        json={
            "message": "Follow up after first result",
            "client_message_id": "runtime-second",
            "task_id": task_id,
        },
    )
    ...
    runtime = second.json()
    assert runtime["task"]["id"] == task_id
    user_messages = [
        event
        for event in runtime["timeline"]
    ...
    assert [
        event["payload"]["message"] for event in user_messages
    ][-1] == "Follow up after first result"
    assistant_message_ids = [
        event["payload"]["message_id"]
        for event in runtime["timeline"]
        if event["type"] == "brain.runtime.message_done"
    ]
    assert assistant_message_ids
    assert assistant_message_ids[-1].startswith("runtime-second:")

    runs = {
        run.client_message_id: run
        for run in (
            await session.scalars(
                select(AgentRun).where(
                    AgentRun.client_message_id.in_(["runtime-first", "runtime-second"])
                )
            )
        )
    }
    assert runs["runtime-first"].task_id == task_id
    assert runs["runtime-second"].task_id == task_id
