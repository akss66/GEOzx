...
- Added member-detail wire normalization because a legacy backend can omit `client_ids`, `project_ids`, `account_ids`, and modern membership collections.
...
- Detail boundary: `getUserDetail` and the access-update response preserve present legacy client/project/account IDs, derive IDs from modern memberships when possible, and mark the canonical detail `access_detail_status: "unavailable"` when the fields required to reconstruct a complete replacement payload are absent.
...
- Legacy-detail follow-up RED: the same targeted command failed the 2 new cases out of 31 total before production changes. One showed the raw legacy detail escaping the API boundary; the other showed the resource tab still exposing an editable replacement form.
- Legacy-detail follow-up GREEN: the targeted command passed 31 of 31 after implementing normalization and the compatibility read-only state.
...
4. `renders legacy incomplete member access as read-only without overwriting authorization`
5. `creates a scoped member with initial resource authorization in one flow`
6. `recovers from a partial create result without creating the identity twice`
7. `creates an administrator without requiring scoped grants`
8. `filters the roster by search, role, anomaly, and status`
9. `saves identity changes and translates enable-disable business errors`
10. `toggles account scope, persists selected accounts, and explains when no accounts are visible`
11. `sets the current admin secondary password and resets the selected member password`
12. `recovers from stale delete previews and clears sensitive destructive inputs`
13. `clears destructive inputs when the delete flow closes`
14. `removes a permanently deleted member from the roster and selects the next member`
15. `isolates security drafts and deletion previews when the selected member changes`
16. `renders global access as read-only and blocks scoped access saves`
17. `preserves unsaved access drafts across overview save and status refetches`
18. `drops an unsaved access draft when the selected member changes`
19. `translates stable business errors from overview saves`
20. `translates generic 422 errors from overview saves`
21. `enables permanent deletion only after exact email and secondary password confirmation`

The targeted API regressions are `preserves an unavailable account catalog when legacy responses omit accounts` and `preserves present legacy access IDs and marks incomplete member detail unavailable` in `frontend/src/api/auth.test.ts`. The layout regression is `member governance fits the 1440px desktop viewport without document overflow` in `frontend/e2e/users-layout.spec.ts`.
...
- Incomplete legacy member details show unknown resource metrics and an explicit compatibility read-only state. No save control is rendered, because the backend access endpoint replaces all memberships.
...
- Missing legacy detail arrays are normalized to safe iterable arrays only alongside `access_detail_status: "unavailable"`; that status prevents the placeholders from being interpreted or submitted as empty authorization. Present legacy IDs remain in the canonical detail.
...
- Focused Users: `npm.cmd test -- src/pages/Users.test.tsx` - passed independently, 1 file / 21 tests.
- Targeted Users + API: `npm.cmd test -- src/pages/Users.test.tsx src/api/auth.test.ts` - passed, 2 files / 31 tests.
- Desktop Chromium: `npx.cmd playwright test e2e/users-layout.spec.ts --project=chromium` - passed, 1 test at 1440 x 900.
- Full frontend: `npm.cmd test` - passed, 52 files / 190 tests.
